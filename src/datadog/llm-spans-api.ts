...
import { ChatAgentContext, ChatMessage } from "../core";
import { EventSubscriber } from "../core/event-subscriber";
...
export class LlmSpansApi {
  ...
  public handleChatStart(event: EventChatStart) {
    ...
    const traceId = tracer.scope().active()?.context().toTraceId() || uuid();
    const parentSpanId = tracer.scope().active()?.context().toSpanId();
